...
//list1.ForEach(dt => Console.WriteLine(dt));
...
//list2.ForEach(dt => Console.WriteLine(dt.BirthDay));

//3) Borcu 5000 - den cox olmayan borclularic cixartmag

var list3 = debtors.Where(dt => dt.Debt <= 5000).ToList();

//list3.ForEach(dt => Console.WriteLine(dt.Debt));

//4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclularicixartmaq

var list4 = debtors.Where(dt => dt.FullName.Length >= 18 && dt.Phone.Count(ch => ch == '7') >= 2).ToList();

//list4.ForEach(dt => Console.WriteLine(dt.Phone + dt.FullName));
...
//list5.ForEach(dt => Console.WriteLine(dt.BirthDay));

//6) Borcu, umumi borclarin orta borcunnan cox olan borclulari cixarmaq ve evvel familyaya gore sonra isemeblegin azalmagina gore sortirovka etmek
...
//list6.ForEach(dt => Console.WriteLine(dt));
...
//list7.ForEach(dt => Console.WriteLine(dt.FullName.Split()[2] + " " + dt.BirthDay + " " + dt.Debt));


//8)Adinda ve familyasinda hec olmasa 3 eyni herf olan borclularin siyahisin cixarmaq ve onlari elifbasirasina gore sortirovka elemek
...
//list9.ForEach(dt => Console.WriteLine(dt.BirthDay.Year));
...
//list13.ForEach(dt => Console.WriteLine(dt.Debt));

//14)Tesevvur edek ki, butun borclari olanlar bugunden etibaren her ay 500 azn pul odeyecekler. Oz adgunune kimi borcun oduyub qurtara bilenlerin siyahisin cixartmaq
...
//list14.ForEach(dt => Console.WriteLine(dt));
...
//list15.ForEach(dt => Console.WriteLine(dt.FullName));
